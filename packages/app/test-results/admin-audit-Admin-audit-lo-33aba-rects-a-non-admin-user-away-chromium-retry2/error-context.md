...
- Name: admin-audit.spec.ts >> Admin audit log page — authorization >> redirects a non-admin user away
- Location: e2e/admin-audit.spec.ts:101:7
...
Expected pattern: not /dashboard\/admin\/audit/
Received string: "http://localhost:3001/en/dashboard/admin/audit"
Timeout: 10000ms

Call log:
  - Expect "not toHaveURL" with timeout 10000ms
    12 × unexpected value "http://localhost:3001/en/dashboard/admin/audit"
    2 × waiting for" http://localhost:3001/en/dashboard/admin/audit" navigation to finish...
      - navigated to "http://localhost:3001/en/dashboard/admin/audit"
      - unexpected value "http://localhost:3001/en/dashboard/admin/audit"
      - unexpected value "http://localhost:3001/en/dashboard/admin/audit"
    3 × unexpected value "http://localhost:3001/en/dashboard/admin/audit"
...
```ts
  5   |  * (dispute resolution, user moderation) produce a corresponding audit
  6   |  * log entry — the key requirement called out in the issue.
  7   |  * The API is mocked via route interception, matching the pattern used by
  8   |  * admin-disputes.spec.ts — this suite runs in CI jobs that only boot the
  9   |  * Next.js app (no live backend).
  10  |  */
  11  | import { test, expect, type Page } from '@playwright/test';
  12  | 
  13  | const BASE = process.env.PLAYWRIGHT_BASE_URL ?? 'http://localhost:3001';
  14  | const MOCK_TOKEN = 'mock-e2e-jwt-token';
  15  | 
  16  | const ADMIN_USER = {
  17  |   id: 'admin-1',
  18  |   firstName: 'Ada',
