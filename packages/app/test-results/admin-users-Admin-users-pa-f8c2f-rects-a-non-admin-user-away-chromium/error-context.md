...
- Name: admin-users.spec.ts >> Admin users page — authorization >> redirects a non-admin user away
- Location: e2e/admin-users.spec.ts:129:7
...
Expected pattern: not /dashboard\/admin\/users/
Received string: "http://localhost:3001/en/dashboard/admin/users"
Timeout: 10000ms

Call log:
  - Expect "not toHaveURL" with timeout 10000ms
    5 × unexpected value "http://localhost:3001/en/dashboard/admin/users"
    2 × waiting for" http://localhost:3001/en/dashboard/admin/users" navigation to finish...
      - navigated to "http://localhost:3001/en/dashboard/admin/users"
      - unexpected value "http://localhost:3001/en/dashboard/admin/users"
      - unexpected value "http://localhost:3001/en/dashboard/admin/users"
    8 × unexpected value "http://localhost:3001/en/dashboard/admin/users"
...
```ts
  ...
  34  |   lastName: string;
  35  |   email: string;
  36  |   role: 'user' | 'curator' | 'admin';
  37  |   deletedAt: string | null;
  38  |   createdAt: string;
  39  | }
  40  | 
  41  | function makeUsers(): MockUser[] {
  42  |   return [
  43  |     {
  44  |       id: 'u-1',
  45  |       firstName: 'Wanda',
