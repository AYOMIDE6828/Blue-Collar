...
- Name: admin-users.spec.ts >> Admin users page — listing and filters >> filters the user list by search term
- Location: e2e/admin-users.spec.ts:150:7
...
> 155 |     await expect(page.getByText('Wanda Worker')).toBeVisible();
  ...
  245 |       return route.fulfill({
  246 |         status: 200,
  247 |         contentType: 'application/json',
  248 |         body: JSON.stringify({ data: { id: 'u-1', suspended: false }, status: 'success', code: 200 }),
  249 |       });
  250 |     });
  251 | 
  252 |     await page.goto(`${BASE}/en/dashboard/admin/users`);
  253 |     const row = page.locator('tr', { hasText: 'Wanda Worker' });
  254 |     await expect(row.getByText('Suspended')).toBeVisible();
  255 | 
```