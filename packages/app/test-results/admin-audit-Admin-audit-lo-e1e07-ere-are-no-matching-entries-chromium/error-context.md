...
- Name: admin-audit.spec.ts >> Admin audit log page — listing and filtering >> shows an empty state when there are no matching entries
- Location: e2e/admin-audit.spec.ts:121:7
...
Locator: getByText('No audit log entries found')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText('No audit log entries found')
